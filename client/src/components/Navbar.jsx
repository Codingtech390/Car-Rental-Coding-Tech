...
const Navbar = ({ setShowLogin }) => {
  ...
  return (
    ...
    >
      ...
      >
        {menuLinks.map((link, indexNo) => (
          <Link key={indexNo} to={link.path}>
            {link.name}
          </Link>
        ))}

        <div className="hidden lg:flex items-center text-sm gap-2 border border-borderColor px-4  rounded-full max-w-56">
          <input
            type="text"
            name=""
